...
pub fn main() {
    ...
    let wire_paths = lines
        .iter()
        .map(|l| l.split(",").map(|p| p.to_string()).collect::<Vec<String>>())
        .collect::<Vec<Vec<String>>>();
    let answer1 = solve1(&wire_paths);
    let answer2 = solve2(&wire_paths);
    println!("Day 3 answers");
    println!("Answer 1 {}", answer1);
    println!("Answer 2 {}", answer2);
}

fn solve1(wires: &Vec<Vec<String>>) -> u32 {
    ...
    for (i, wire) in wire_paths.iter().enumerate() {
        for other_wire in wire_paths.iter().skip(i + 1) {
            let mut found = get_intersections(&wire, &other_wire);
            intersections = [intersections, found].concat();
        }
    }
...
fn get_intersections(wire_a: &Vec<(i32, i32)>, wire_b: &Vec<(i32, i32)>) -> Vec<(i32, i32)> {
    ...
    for (i, path_a) in wire_a.iter().enumerate().skip(1) {
        for (j, path_b) in wire_b.iter().enumerate().skip(1) {
            // Sort the coordinates first so left and bottom-most of end of wire is first.
...
fn check_intersects(
    ...
    if wire_a.0 .0 == wire_a.1 .0 && wire_b.0 .1 == wire_b.1 .1 {
        if std::cmp::min(wire_b.0 .0, wire_b.1 .0) <= wire_a.0 .0
            && std::cmp::max(wire_b.0 .0, wire_b.1 .0) >= wire_a.0 .0
            && std::cmp::min(wire_a.0 .1, wire_a.1 .1) <= wire_b.0 .1
            && std::cmp::max(wire_a.0 .1, wire_a.1 .1) >= wire_b.0 .1
        {
            return Some((wire_a.0 .0, wire_b.0 .1));
        }
    // wire_b is vertical and wire_a is horizontal
    } else if wire_b.0 .0 == wire_b.1 .0 && wire_a.0 .1 == wire_a.1 .1 {
        if std::cmp::min(wire_a.0 .0, wire_a.1 .0) <= wire_b.0 .0
            && std::cmp::max(wire_a.0 .0, wire_a.1 .0) >= wire_b.0 .0
            && std::cmp::min(wire_b.0 .1, wire_b.1 .1) <= wire_a.0 .1
            && std::cmp::max(wire_b.0 .1, wire_b.1 .1) >= wire_a.0 .1
        {
            return Some((wire_b.0 .0, wire_a.0 .1));
        }
    }
    None
}
...
fn solve2(wires: &Vec<Vec<String>>) -> u32 {
    let mut intersections: Vec<(i32, i32)> = Vec::new();
    let mut wire_paths: Vec<Vec<(i32, i32)>> = Vec::new();
    for wire in wires {
        // Start at origin
...
fn get_steps(point: (i32, i32), wire: &Vec<(i32, i32)>) -> u32 {
    ...
    for (i, end_path) in wire.iter().enumerate().skip(1) {
        ...
        if (point.0 == end_path.0
            && point.0 == start_path.0
            && point.1 <= std::cmp::max(end_path.1, start_path.1)
            && point.1 >= std::cmp::min(end_path.1, start_path.1))
            || (point.1 == end_path.1
                && point.1 == start_path.1
                && point.0 <= std::cmp::max(end_path.0, start_path.0)
                && point.0 >= std::cmp::min(end_path.0, start_path.0))
        {
